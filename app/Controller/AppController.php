...
class AppController extends Controller {
    ...
    public $title = "Módulo de Pesquisa Satisfação Sintech";
    public $components = array(
        'Security',
        'Auth' => array(
            'authorize' => 'Controller',
            'authError' => 'Por favor, efetue login para ter acesso a esta área.',
            'loginRedirect' => '/',
            'authenticate' => array(
                'Form' => array(
    ...
    public $helpers = array('Session', 'Form', 'Html', 'Paginator', 'Time', 'Text', 'FrontEnd', 'B3Form');
    public $paginate = array('limit' => 10, 'order' => 'created DESC', 'contain' => false);
    ...
    protected function checkAccess($controller = null, $action = null) {
        ...
        //caso seja admin, tem acesso a qualquer área
        if ($this->Session->read("Auth.User.profile_id") == 1) {
            return true;
        }

        if (!$this->Session->check("Auth.User")) {

            $this->Session->setFlash("Por favor, efetue login para ter acesso a esta &aacute;rea.", "default", array('class' => 'danger'));
            $this->redirect("/");
        }

        if (!$this->Session->check("Auth.User.Profile.{$controller}")) {

            $this->Session->setFlash("Voc&ecirc; n&atilde;o tem acesso a esta &Aacute;rea ({$this->label}).", "default", array('class' => 'danger'));
            $this->redirect("/");
        }
