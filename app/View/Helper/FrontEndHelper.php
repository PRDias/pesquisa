...
class FrontEndHelper extends AppHelper
{
    ...
    public function niceDate($date, $verbose = false)
    {
        if (empty($date))
            return '';
        if ($verbose)
            return '<i class="fa fa-calendar"></i> ' . $this->Time->format("d", $date) . ' de ' . $this->months[$this->Time->format("n", $date)] . ' de ' . $this->Time->format("Y", $date) . ' <i class="fa fa-clock-o"></i> ' . $this->Time->format("H:i:s", $date);

        return '<i class="fa fa-calendar"></i> ' . $this->Time->format("d/m/Y", $date) . ' <i class="fa fa-clock-o"></i> ' . $this->Time->format("H:i:s", $date);
    }
        ...
        foreach ($areas as $area) {
            ...
            if (!empty($permissions[$area['controller']]['action'][$area['action']])) {
                ...
                if (empty($area['AreaChild'])) {
                    $icon = empty($area['icon_menu']) ? "" : "<i class=\"{$area['icon_menu']}\"></i> ";

                    $string .= '<li class="' . $this->optionSelected($area['controller']) . '">'  . $this->Html->link($icon.$area['controller_label'], "/{$area['controller']}/{$area['action']}", array('escape' => false)) . "</li>\n";
                } else { // submenu
                    $icon = empty($area['icon_menu']) ? "" : "<i class=\"{$area['icon_menu']}\"></i> ";
                    $string .= '<li class="dropdown">' .
                        '<a href="#" class="dropdown-toggle" data-toggle="dropdown">' . $icon . $area['controller_label'] . ' <b class="caret"></b></a>' .
